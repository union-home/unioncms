                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="setting-header">
                                ...
                                <h6 class="d-inline">解绑手机号</h6>
                                @if(!empty($login_user['phone']))
                                    <span class="bind-email">已绑定： {{$login_user['phone']}}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="userinfo-content mt-md-3 mt-3">
                        <form method="post" action="" id="post_form" enctype="multipart/form-data">
                            ...
                            <div class="col-12 mt-md-3">
                                <div class="">
                                    <div class="userinfo-group w-100 email">
                                        <span class="userinfo-text">手机号码：</span>
                                        <input class="bc-2nd-f7 d-md-inline-block d-block w-25 mt-2 border px-2 py-1" type="text" name="phone" value="{{$login_user['phone']}}" readonly="" />
                                        <span class="form-control bc-2nd-f7 d-md-inline-block d-block w-25 mt-2 border px-2 py-1" id="send_phone_button">
                                            发送验证码
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="col-12 mt-md-3">
                                <div class="">
                                    <div class="userinfo-group w-100">
                                        ...
                                        <input class="bc-2nd-f7 d-md-inline-block d-block w-25 mt-2 border px-2 py-1" name="phone_code" type="text" placeholder="请输入验证码">
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 my-3 d-flex justify-content-md-start justify-content-around">
                                ...
                                <input class="save-btn py-2 px-5 bc-theme border-0 text-white" type="button" value="解除手机号绑定" id="post_button">
                            </div>
                        </form>
                    </div>
                </li>
...
<script type="text/javascript">
    $("#post_button").click(function () {
        popup({type:'load',msg:"正在请求",delay:800,callBack:function(){
            $.ajax({
                "method":"post",
                "url":"{{url('member/safetySubmit?form=untying_phone')}}",
                ...
                "success":function (res) {
                    if(res.status==200){
                        ...
                        setTimeout(function () {
                            location.href="{{url('member/setting')}}";
                        },2000);
                    }else{
    ...
    $("#send_phone_button").click(function () {
        popup({type:'load',msg:"正在请求",delay:800,callBack:function(){
            $.ajax({
                "method":"post",
                "url":"{{url('member/safetySubmit?form=send_untying_phone')}}",
                "data": new FormData($('#post_form')[0]),                    //$("#post_form").serialize(),
                "dataType":'json',
                "cache":false,
                "processData": false,
