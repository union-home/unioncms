...
if (isset($sysInfo['swoole_loader']) and isset($sysInfo['swoole_loader_version'])) {
    $env['php']['swoole_loader']['status']  = $sysInfo['swoole_loader'] ? "<span style='color: #007bff;'>已安装</span> &nbsp; &nbsp; <a href='/'>去安装</a>"
        : '未安装';
...
if ('web' == $env['php']['run_mode']) {
    ...
    if (empty($html_error)) {
        ...
        $html_body_loader .= '<p><span class="bold_text">1 - 下载Swoole Loader</span></p><p>请下载兼容PHP' . $env['php']['version'] . '和' . $env['php']['thread_safety'] . '的Swoole Loader扩展，<a target="_blank" href="http://www.unioncms.cn/member/download">点击直达下载页面</a></p>';
        $html_body_loader .= '<p><span class="bold_text">2 - 安装Swoole Loader</span></p><p>将刚才下载的Swoole Loader扩展文件（swoole_loader.dll或swoole_loader.so）上传到当前PHP的扩展安装目录中：<br/><pre class="code">' . $env['php']['extension_dir'] . '</pre></p>';
        ...
        if ($env['os']['name'] ==  "windows") {
            $html_body_loader .= '<pre class="code">extension=swoole_loader.dll</pre>注意：需要名称和刚才上传到当前PHP的扩展安装目录中的文件名一致';
        } else {
            $html_body_loader .= '<pre class="code">extension=swoole_loader.so</pre>注意：需要名称和刚才上传到当前PHP的扩展安装目录中的文件名一致';
        }
        $html_body_loader .= '</p>';
	...
	<p class="mb-1">技术支持交流群：<a target="_blank" href="https://jq.qq.com/?_wv=1027&k=5dj64k8">614731256</a>（CMS）<a target="_blank" href="https://jq.qq.com/?_wv=1027&k=51maBBa">279148972</a>（开发者）</p>
  </footer>';
